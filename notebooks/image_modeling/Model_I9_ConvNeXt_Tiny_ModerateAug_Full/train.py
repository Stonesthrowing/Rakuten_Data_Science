...
#
# Model I9 specifics:
#   - Architecture  : ConvNeXt-Tiny (full fine-tuning)
...
def main() -> None:
    ...
    # ConvNeXt label mapping uses string keys – keep consistent with ResNet models
    ...
    def make_local_image_path(row) -> str:
        fname = f"image_{row['imageid']}_product_{row['productid']}.jpg"
        return str(config.LOCAL_IMAGE_TRAIN_DIR / fname)
    ...
    # ----------------------------------------------------------
    # 4. Transforms  (TrivialAugmentWide for train)
    # ----------------------------------------------------------
    ...
    train_loader = DataLoader(
        RakutenImageDataset(train_df, transform=train_transform),
    ...
    # ----------------------------------------------------------
    # 6. Model  (ConvNeXt-Tiny, full fine-tuning)
    # ----------------------------------------------------------
    ...
    # Differential learning rates: smaller for backbone to protect pretrained weights
    ...
    # ----------------------------------------------------------
    print("\n" + "=" * 60)
    print("Starting Training  (ConvNeXt-Tiny – Full Fine-Tuning) …")
    print("=" * 60)
