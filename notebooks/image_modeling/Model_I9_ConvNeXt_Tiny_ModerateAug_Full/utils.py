...
def save_run_metadata(path: Path, model_name: str, best_f1: float, best_epoch: int, duration_min: float) -> None:
    """Save a small JSON with key run results for easy comparison across experiments."""
    meta = {
        "model_name":    model_name,
...
def run_epoch(model, loader, criterion, device, optimizer=None, scaler=None) -> tuple:
    ...
    Args:
        ...
        scaler    : torch.cuda.amp.GradScaler instance for mixed-precision
                    training. Pass None to disable AMP.
...
def plot_and_save_history(history: list, fig_dir: Path, best_epoch: int, best_f1: float) -> None:
    ...
    try:
        ...
        fig.suptitle(f"Training Curves  (Best Epoch: {best_epoch} | Best Macro-F1: {best_f1:.4f})", fontsize=13)
...
def plot_confusion_matrix(
    ...
    Args:
        all_true     : Ground-truth integer labels.
        all_preds    : Predicted integer labels.
        target_names : Class name strings ordered by label index.
        fig_dir      : Directory where the PNG is saved.
        model_name   : Used in the plot title.
        best_f1      : Best macro-F1 shown in the title.
    """
    fig_dir.mkdir(parents=True, exist_ok=True)
    ...
    plt.title(f"Normalised Confusion Matrix – {model_name}  (Macro-F1: {best_f1:.4f})", fontsize=14)
...
def evaluate_model(
    ...
    """
    Run inference on loader, print a classification report, save the
    confusion matrix, and return a metrics dict.
