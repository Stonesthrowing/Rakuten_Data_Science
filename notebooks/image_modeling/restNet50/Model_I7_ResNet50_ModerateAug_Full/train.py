...
#
# Model I7 specifics:
#   - Architecture : ResNet50 (full fine-tuning – all layers unfrozen)
#   - Lower LR (1e-4) to avoid destroying pretrained weights
# ============================================================
...
from notebooks.image_modeling.restNet50.Model_I7_ResNet50_ModerateAug_Full.utils import (
...
def main() -> None:
    ...
    # ----------------------------------------------------------
    # 6. Model  (ResNet50, FULL unfreeze – all layers trainable)
    # ----------------------------------------------------------
    ...
    # All parameters passed to optimizer (full fine-tuning)
    ...
    # ----------------------------------------------------------
    print("\n" + "=" * 60)
    print("Starting Training  (ResNet50 – Full Fine-Tuning) …")
    print("=" * 60)
    start_time = time.time()

    for epoch in range(start_epoch, config.MAX_EPOCHS + 1):

        train_loss, train_acc, train_f1, train_weighted_f1 = run_epoch(
            model, train_loader, criterion, device, optimizer
        )
        val_loss, val_acc, val_f1, val_weighted_f1 = run_epoch(
