...
def gradcam_observation(row):
    ...
    specific = {
        ("1280", "2583"): "Attention is diffuse and partly falls on the background rather than the product itself — a hallmark of the low-confidence wrong prediction the model produces for Toys/Plush. The class is one of the hardest in the dataset (val F1 0.35).",
        ("1302", "1560"): "The heatmap focuses on an object detail that is visually consistent with home furniture rather than an outdoor game — the model's attention is plausible but misaligned. Outdoor Games (1302) scores only F1 0.43 on the validation set.",
        ("2060", "2522"): "Activation concentrates near the image edges and packaging graphics rather than the central product silhouette, suggesting that ambient context or printed text is driving the stationery prediction instead of product shape.",
        ("1280", "2522"): "Attention is localised to a small subset of items in the image rather than the overall product group. The framing makes the product resemble stationery — consistent with the 1280 → 2522 error pattern seen in the confusion matrix.",
        ("1560", "2522"): "The model attends strongly to the flat front panel of the item. A shelf or door panel reads visually like stationery storage, explaining a confident but wrong prediction. Furniture (1560) → Lamps/Decor (2060) and Stationery (2522) are the top two error targets for this class.",
        ("1320", "1320"): "Correct prediction but uncertain (conf 0.149). The heatmap shows scattered attention across the product rather than a sharp focus — Women's Bags (1320, val F1 0.46) is a challenging class because bags, pouches and early-childhood accessories share similar shapes and colours.",
        ("1302", "1302"): "Strong, localised attention on the game equipment with very high confidence (0.999). Outdoor Games products have distinctive shapes and colour patterns that ConvNeXt-Base recognises reliably when the image is clear.",
    }
...
if page not in NAV_ITEMS :
    ...
    exp_df = pd.DataFrame([
        {"ID": "I5", "Strategy": "Frozen backbone",      "Augmentation": "No",       "Unfrozen layers": "Head only",    "LR": "1e-4", "Batch": "32",  "Dropout": "—",   "Label smoothing": "No"},
        {"ID": "I6", "Strategy": "Partial unfreezing",   "Augmentation": "Moderate", "Unfrozen layers": "Last block + head", "LR": "3e-4", "Batch": "128", "Dropout": "0.5", "Label smoothing": "0.1"},
        {"ID": "I7", "Strategy": "Full unfreezing",      "Augmentation": "Moderate", "Unfrozen layers": "All",          "LR": "1e-4", "Batch": "128", "Dropout": "0.5", "Label smoothing": "0.1"},
        {"ID": "I8", "Strategy": "Random initialisation","Augmentation": "Moderate", "Unfrozen layers": "All",          "LR": "3e-4", "Batch": "16",  "Dropout": "0.3", "Label smoothing": "No"},
    ])
    ...
    if not resnet_i6_history.empty or not resnet_i7_history.empty:
        ...
        for ax_c, hist, title, c_train, c_val in [
            ...
            ax_c.plot(epochs, hist["train_macro_f1"], color=c_train, lw=1.5, linestyle="--", label="Train macro F1", alpha=0.7)
    ...
    render_html_table(pd.DataFrame([
        {"Model": "I5 — Frozen",           "Macro F1": "0.554", "Key observation": "Strong ImageNet features, zero adaptation. Head converges but backbone is bottlenecked at generic features."},
        {"Model": "I6 — Partial unfreeze", "Macro F1": "0.641", "Key observation": "+0.087 over I5. Unfreezing the last residual block lets the backbone adapt its highest-level features to Rakuten's product styles. Higher LR (3e-4) is safe because only one block changes."},
        {"Model": "I7 — Full unfreeze",    "Macro F1": "0.653", "Key observation": "+0.012 over I6. Full fine-tuning squeezes a small extra gain. The lower LR (1e-4) protects against catastrophic forgetting of early-layer edge detectors that are still useful here."},
    ]), max_width="960px")
    st.write(
    ...
    st.subheader("Experiments overview")
    exp_df = pd.DataFrame([
        {"ID": "I9",  "Backbone": "ConvNeXt-Tiny",  "Params": "~28 M", "Augmentation": "Moderate", "Unfrozen layers": "All", "LR (backbone / head)": "1e-4 / 1e-4", "Batch": "32",  "Best epoch": "19", "Val macro F1": "0.685"},
        {"ID": "I12", "Backbone": "ConvNeXt-Base",  "Params": "~89 M", "Augmentation": "Moderate", "Unfrozen layers": "All", "LR (backbone / head)": "5e-6 / 5e-5", "Batch": "16",  "Best epoch": "20", "Val macro F1": "0.692"},
    ])
    render_html_table(exp_df, max_width="960px")
    ...
    if not _i9_norm.empty or not _i12_norm.empty:
        ...
        for ax_c, hist_n, title, c_train, c_val in [
            ...
            ax_c.plot(epochs, hist_n["train_macro_f1"], color=c_train, lw=1.5, linestyle="--", alpha=0.7, label="Train macro F1")
    ...
    if _cm_labeled.exists():
        st.image(str(_cm_labeled), caption="Row-normalised confusion matrix with class names and counts — ConvNeXt-Base (I12)", use_container_width=True)
    elif FILES["confusion_png"]:
        st.image(str(FILES["confusion_png"]), caption="Normalised validation confusion matrix — ConvNeXt-Base (I12)", use_container_width=True)
    else:
    ...
    render_html_table(pd.DataFrame([
        ...
        {"Metric": "Best epoch",        "Value": "20 / 20", "Note": "No early stopping — model still improving at the end of training"},
        {"Metric": "Training strategy", "Value": "Differential LR", "Note": "Backbone 5e-6, head 5e-5 — preserves pretrained features while adapting to 27 Rakuten classes"},
        {"Metric": "Architecture",      "Value": "~89 M params", "Note": "ConvNeXt-Base with 7×7 depthwise conv, GELU, LayerNorm — richer representations than ResNet at similar cost"},
    ]), max_width="960px")
...
# ===================================================================
    ...
    comp_df = pd.DataFrame([
        {"Model": "Text only — CamemBERT",                                          "Macro F1": _f(mm_late_meta.get("f1_text_only"))},
        {"Model": "Image only — ConvNeXt-Base",                                     "Macro F1": _f(mm_late_meta.get("f1_image_only"))},
        {"Model": f"Late Fusion (α = {mm_late_meta.get('best_alpha', '—')})",       "Macro F1": _f(mm_late_meta.get("best_macro_f1"))},
...
(frozen → staged unfreeze → staged unfreeze + image augmentation + label smoothing).
    ...
    render_html_table(pd.DataFrame([
        ...
        {"Strategy": "Intermediate Fusion (trained head)",  "Macro F1": _f(inter_f1), "Notes": "Feature concat + joint linear head, 6 epochs"},
        {"Strategy": "CLIP Gated Fusion (best run)",        "Macro F1": "0.8800",     "Notes": "CamemBERT + CLIP ViT-B/32, augmentation + label smoothing"},
        {"Strategy": "Simple Fusion — Late Fusion (alpha-sweep)", "Macro F1": _f(macro_f1), "Notes": "Weighted average of softmax outputs, no training"},
    ]), max_width="960px")
    ...
    _cls_names = ["Books","Video Games","Game Acc.","Game Consoles","Figurines","Coll. Cards","Board Figs.",
                  ...
                  "Console/Games","Stationery","Outdoor Furn.","Pool Acc.","Tools/Garden","Comics/Books","DL Games"]
